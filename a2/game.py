...
class Game:
    ...
    def __init__(self, max_depth: int,
        ...
        random_goal_type = random.randint(0, 1)
        ...
        for i in range(num_human, random_players + num_human):
            colour = COLOUR_LIST[random.randint(0, 3)]
            if random_goal_type == 0:
                goal = BlobGoal(colour)
            else:
        ...
        total_num_of_player = num_human + random_players + len(smart_players)
        for i in range(num_human + random_players, total_num_of_player):
            colour = COLOUR_LIST[random.randint(0, 3)]
            if random_goal_type == 0:
                goal = BlobGoal(colour)
            else:
